...
public class PixmapPacker implements Disposable {
	...
	public synchronized Rectangle pack (String name, Pixmap image) {
		...
		Rectangle rect;
		if (isPatch) {
			rect = new Rectangle(0, 0, image.getWidth() + borderPixels - 2, image.getHeight() + borderPixels - 2);
			Pixmap pix = new Pixmap(image.getWidth() + borderPixels - 2, image.getHeight() + borderPixels - 2, image.getFormat());
			rect.splits = getSplits(image);
			pix.drawPixmap(image, 0, 0, 1, 1, image.getWidth() + borderPixels - 1, image.getHeight() + borderPixels - 1);
			image = pix;
			name = name.split("\\.")[0];
			splits = rect.splits;
		} else {
			rect = new Rectangle(0, 0, image.getWidth() + borderPixels, image.getHeight() + borderPixels);
		}
		...
		if (splits != null) rect.splits = splits;
		...
		for (Page page : pages) {
			if (page.addedRects.size > 0) {
				for (String name : page.addedRects) {
					...
					if (rect.splits != null) {
						region = new TextureAtlas.AtlasRegion(page.texture, (int)rect.x, (int)rect.y, (int)rect.width, (int)rect.height);
						((TextureAtlas.AtlasRegion) region).splits = rect.splits;
						((TextureAtlas.AtlasRegion) region).name = name;
						atlas.getRegions().add((TextureAtlas.AtlasRegion) region);
	...
	static private int getSplitPoint (Pixmap raster, int startX, int startY, boolean startPoint, boolean xAxis) {
		int[] rgba = new int[4];
